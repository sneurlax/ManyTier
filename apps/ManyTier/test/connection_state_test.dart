...
void main() {
  ...
  group('connectionsLoaderProvider', () {
    ...
    test('restores a previously persisted list and active id', () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'manytier.connections': '[{"id":"a","label":"A","host":"1.2.3.4","port":1000},'
    ...
    test('falls back to the first connection when the active id is stale',
        () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'manytier.connections': '[{"id":"a","label":"A","host":"1.2.3.4","port":1000}]',
    ...
    test('falls back to the default connection on corrupt persisted JSON',
        () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
      ...
      expect(container.read(connectionsProvider), <SavedConnection>[defaultConnection]);
    });
  });
  ...
  group('ConnectionsController', () {
    test('add appends and switches to the new connection', () async {
      ...
      const added = SavedConnection(id: 'new', label: 'New', host: '10.0.0.1', port: 42);
      container.read(connectionsControllerProvider).add(added);

      expect(container.read(connectionsProvider), <SavedConnection>[defaultConnection, added]);
    ...
    test('switchTo changes the active id when it exists', () async {
      ...
      addTearDown(container.dispose);
      await container.read(connectionsLoaderProvider.future);
      container.read(connectionsControllerProvider).add(
            const SavedConnection(id: 'new', label: 'New'),
          );

      container.read(connectionsControllerProvider).switchTo(defaultConnection.id);
    ...
    test('forget removes a connection and reassigns active if it was active',
        () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      await container.read(connectionsLoaderProvider.future);
    ...
    test('forget clears active id when the last connection is removed',
        () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      await container.read(connectionsLoaderProvider.future);

      container.read(connectionsControllerProvider).forget(defaultConnection.id);
